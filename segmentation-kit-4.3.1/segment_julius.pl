...
foreach $f (@files) {
    ...
    $flog = $f;
    $fdfa_dict = $f;
    $flab =~ s/16kHz/segmentation-kit-4.3.1\/lab/g; 
    $flog =~ s/16kHz/segmentation-kit-4.3.1\/log/g;
    $fdfa_dict =~ s/16kHz/segmentation-kit-4.3.1\/dfa_dict/g;
    # generate dfa and dict for Julius
    unlink("$fdfa_dict.dfa") if (-r "$fdfa_dict.dfa");
    unlink("$fdfa_dict.dict") if (-r "$fdfa_dict.dict");
    open(DFA, "> $fdfa_dict.dfa") || die "Error: cannot open $fdfa_dict.dfa for writing\n";
    for ($i = 0; $i <= $num; $i++) {
    ...
    print DFA "$str";
    close(DFA);
    open(DICT, "> $fdfa_dict.dict") || die "Error: cannot open $fdfa_dict.dict for writing\n";
    for ($i = 0; $i <= $num; $i++) {
	$w = shift(@words);
	$str = "$i [w_$i] $w\n";
	$WLIST{"w_$i"} = "$w";
    ...
    close(DICT);
    if ((! -r "$fdfa_dict.dfa") || (! -f "$fdfa_dict.dfa")) {
	die "Error: failed to make \"$fdfa_dict.dfa\"\n";
    }
    if ((! -r "$fdfa_dict.dict") || (! -f "$fdfa_dict.dict")) {
	die "Error: failed to make \"$fdfa_dict.dict\"\n";
    }
    #### execute Julius and store the output to log
    
    
    $command = "echo $fw.wav | $juliusbin -h $hmmdefs -dfa $fdfa_dict.dfa -v $fdfa_dict.dict";
    $command .= " -palign";
    if ($hlist ne "") {
	$command .= " -hlist $hlist";
    }
    ...
    open(RESULT, "> $flab.lab") || die "Error: cannot open result file \"$flab.lab\"\n";
    open(LOG, "$flog.log") || die "Error: cannot open $flog.log file\n";
    $sw = 0;
    while(<LOG>) {
	chomp;
    ...
    print "Result saved in \"$flab.lab\".\n";
}
